...
def logs():
        ...
        if not os.path.exists('/var/log/kasper.log'): 
            f = open('/var/log/kasper.log', "w+")
            f.close()
...
for var, g in enumerate(KSC, 1):
    ...
    if b == 0:
        ...
        if a == n and (var == 1 or var != 1) and d not in c:
            os.system('echo $(date +"%Y%m%d-%H%M%S") Agent KSC connected to server '+g+' >> /var/log/kasper.log')
            exit()
            ...
            if subprocess.call(["/opt/kaspersky/klnagent64/bin/klmover", "-address", g]) == 0: 
                os.system('echo $(date +"%Y%m%d-%H%M%S") KSC Server change operation on '+g+' Successful >> /var/log/kasper.log')
            else:
                os.system('echo $(date +"%Y%m%d-%H%M%S") KSC Server change operation on '+g+' Unsuccessful >> /var/log/kasper.log')
                exit()
            os.system('sleep 5s')
            c, d = corrconn()
            ...
            if subprocess.call(["/opt/kaspersky/klnagent64/bin/klmover", "-address", a]) == 0: 
                os.system('echo $(date +"%Y%m%d-%H%M%S") KSC Server change operation on '+a+' Successful >> /var/log/kasper.log')
                exit()
            else:
                os.system('echo $(date +"%Y%m%d-%H%M%S") KSC Server change operation on '+a+' Unsuccessful >> /var/log/kasper.log')
                exit()
        elif d not in c:
            exit()
        elif var != len(KSC):
            ...
            os.system('echo $(date +"%Y%m%d-%H%M%S") KSC Servers '+str(KSC)+' unavailable >> /var/log/kasper.log')
            exit()
